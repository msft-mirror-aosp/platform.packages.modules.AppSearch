...
public class SafeParcelProcessor extends AbstractProcessor {
          ...
          if (annotatedElement instanceof TypeElement
            ...
            mMessager.printMessage(
                ...
                "Undefined annotation used on "
                    ...
                    + " is returned by RoundEnvironment.getElementsAnnotatedWith.");
          }
        }
      }
    }
  ...
  private boolean processParcelableClass(TypeElement parcelableClass) {
    ...
    if (constructor == null) {
      mMessager.printMessage(
          ...
          "SafeParcelable class must have exactly"
              + " one constructor annotated with @Constructor, which is used to construct"
    ...
    List<VariableElement> fields = ElementFilter.fieldsIn(mElements.getAllMembers(parcelableClass));
    ...
    for (VariableElement field : fields) {
      ...
      SafeParcelable.Indicator indicatorInfo = field.getAnnotation(SafeParcelable.Indicator.class);
      ...
      if (fieldInfo != null && versionFieldInfo != null) {
        ...
        if (usedIds.contains(fieldInfo.id())) {
          ...
          mMessager.printMessage(
              Diagnostic.Kind.ERROR, "Found a duplicate field id=" + fieldInfo.id() + ".", field);
          isOkay = false;
        } else if (reservedIds.contains(fieldInfo.id())) {
          // Found a field with a reserved id
          mMessager.printMessage(
              Diagnostic.Kind.ERROR, "Found a reserved field id=" + fieldInfo.id() + ".", field);
          isOkay = false;
        ...
        if (versionFieldFound) {
          mMessager.printMessage(
              ...
              "More than one field has been"
                  + " annotated with VersionField.  One and only one member may be"
        ...
        if (indicatorFieldFound) {
          mMessager.printMessage(
              Diagnostic.Kind.ERROR,
              "More than one field has been"
                  + " annotated with Indicator.  Only one member may be annotated with"
                  + " Indicator.",
              field);
          isOkay = false;
        }
    ...
    if (isOkay) {
      ...
      mMessager.printMessage(
          ...
          "Errors prevented the SafeParcelable "
              ...
              + " from being processed. Look above this line in the log to find the error.");
    }
  ...
  private boolean processParcelableConstructor(
    ...
    if (modifiers.contains(Modifier.PRIVATE) || modifiers.contains(Modifier.PROTECTED)) {
      mMessager.printMessage(
          ...
          "A SafeParcelable class constructor"
              + " annotated with @Constructor must be either public or package private.",
          constructor);
      isOkay = false;
    }
      ...
      if (paramInfo == null) {
        if (cl.indicatorField != null) {
          ...
          if (indicatorInfo != null) {
            ...
            if (foundIndicator) {
              ...
              mMessager.printMessage(
                  Diagnostic.Kind.ERROR,
                  "Found more than two"
                      + " parameters in the constructor that have been annoatated"
                      + " with @Indicator",
                  parameter);
              isOkay = false;
            }
            ...
            mMessager.printMessage(
                ...
                "All parameters of the constructor"
                    + " annotated with @Constructor must be annotated with @Param,"
          ...
          if (removedParamInfo != null) {
            ...
            if (!usedParamIds.add(paramId)) {
              mMessager.printMessage(
                  ...
                  "@RemovedParam(id="
                      + paramId
                      + ") has"
                      + " already been used.  Each parameter must map to a distinct field"
                      + " id.",
                  parameter);
              isOkay = false;
            } else {
              if (!reservedIds.contains(paramId)) {
                mMessager.printMessage(
                    ...
                    "@RemovedParam(id="
                        + paramId
                        + ") not found as a reserved id. Every @RemovedParam must be listed as a "
                        + "@Reserved id as well.",
                    parameter);
                isOkay = false;
              } else {
                parseField(
                    ...
                    false);
                ParcelableField field = Objects.requireNonNull(cl.fields.get(paramId));
            ...
            // A parameter in this constructor does not have a Param annotation.  This is an
            // error.
            mMessager.printMessage(
                Diagnostic.Kind.ERROR,
                "All parameters of the"
                    + " constructor annotated with @Constructor must be annotated with"
    ...
    if ((cl.indicatorField == null && parcelableConstructor.parameters.size() != cl.fields.size())
  ...
  private void parseField(
    ...
    if (isVersionField) {
      ...
      if (!fieldModifiers.contains(Modifier.FINAL)) {
        mMessager.printMessage(Diagnostic.Kind.ERROR, "Version field must be final.", field);
      }
    }
    if (fieldModifiers.contains(Modifier.PROTECTED) || fieldModifiers.contains(Modifier.PRIVATE)) {
      // field must have a getter
      if (getter == null) {
        mMessager.printMessage(
            Diagnostic.Kind.ERROR, "A private or protected field" + " must have a getter.", field);
      }
    }
      ...
      if (defaultValueUnchecked != null) {
        ...
        mMessager.printMessage(
            ...
            "Both defaultValue and"
                + " defaultValueUnchecked have been specified in the Field annotation.  You"
        ...
        if (!allowsDefaultValue(fieldType)) {
          mMessager.printMessage(
              ...
              "defaultValue in Field annotation"
                  + " is allowed only for primitive types, primitive type object wrappers"
                  + " and String.",
              field);
          resolvedDefaultValue = null;
        } else {
          ...
          if (resolvedDefaultValue == null) {
            mMessager.printMessage(
                ...
                "defaultValue in Field"
                    + " annotation must be a proper value of annotated field's type.",
                field);
          }
        }
      }
    ...
    if (sm != null) {
      ...
      mMessager.printMessage(
          ...
          fieldType
              ...
              + " class implementing SafeParcelable, or a Parcelable class in the Android"
              + " framework.",
          field);
    }
  }
   ...
   * defaultValue parameter is allowed only in @Field annotating field with one of following types:
   * primitive type, primitive type object wrapper or String.
   */
      ...
      if (kind == TypeKind.BOOLEAN
        ...
        try {
          if (kind == TypeKind.DOUBLE
              || (kind == TypeKind.DECLARED && mTypes.isSameType(type, mDoubleType))) {
            ...
            Byte.parseByte(value);
              ...
              || (kind == TypeKind.DECLARED && mTypes.isSameType(type, mIntegerType))) {
  ...
  private SerializationMethods getSerializationMethod(VariableElement field, TypeMirror type) {
    switch (type.getKind()) {
      case BOOLEAN:
        return new SerializationMethods("writeBoolean", false, true, "readBoolean", null, false);
      case BYTE:
      ...
      case DOUBLE:
        return new SerializationMethods("writeDouble", false, true, "readDouble", null, false);
      case FLOAT:
        return new SerializationMethods("writeFloat", false, true, "readFloat", null, false);
      case INT:
      ...
      case SHORT:
        return new SerializationMethods("writeShort", false, true, "readShort", null, false);
      case DECLARED:
        {
          final DeclaredType dt = (DeclaredType) type;
          if (mTypes.isSameType(type, mStringType)) {
            return new SerializationMethods("writeString", false, true, "createString", null, true);
            ...
            return new SerializationMethods("writeIBinder", false, true, "readIBinder", null, true);
          } else if (mTypes.isSameType(type, mBundleType)) {
            return new SerializationMethods("writeBundle", false, true, "createBundle", null, true);
          } else if (mTypes.isSameType(type, mParcelType)) {
            return new SerializationMethods("writeParcel", false, true, "createParcel", null, true);
          } else if (mTypes.isAssignable(mTypes.erasure(type), mTypes.erasure(mListType))) {
            List<? extends TypeMirror> typeArgs = dt.getTypeArguments();
            if (typeArgs.isEmpty() || mTypes.isSameType(mListType, type)) {
              if (isListSafeForReflection((DeclaredType) field.asType())) {
                return new SerializationMethods(
                    "writeList", false, false, "readList", "getClass().getClassLoader()", true);
              } else {
                mMessager.printMessage(
                    Diagnostic.Kind.ERROR,
              ...
              if (mTypes.isAssignable(typeArg, mBooleanType)) {
                return new SerializationMethods(
                    "writeBooleanList", false, true, "createBooleanList", null, true);
              } else if (mTypes.isAssignable(typeArg, mIntegerType)) {
                return new SerializationMethods(
                    "writeIntegerList", false, true, "createIntegerList", null, true);
                ...
                return new SerializationMethods(
                    "writeFloatList", false, true, "createFloatList", null, true);
              } else if (mTypes.isAssignable(typeArg, mDoubleType)) {
                return new SerializationMethods(
                    "writeDoubleList", false, true, "createDoubleList", null, true);
              } else if (mTypes.isAssignable(typeArg, mStringType)) {
                return new SerializationMethods(
                    "writeStringList", false, true, "createStringList", null, true);
              } else if (mTypes.isSameType(typeArg, mParcelType)) {
                return new SerializationMethods(
                    "writeParcelList", false, true, "createParcelList", null, true);
              } else if (mTypes.isAssignable(typeArg, mIBinderType)) {
                return new SerializationMethods(
                    "writeIBinderList", false, true, "createIBinderList", null, true);
            ...
            return new SerializationMethods(
                "writeSparseBooleanArray", false, true, "createSparseBooleanArray", null, true);
          } else if (mTypes.isAssignable(type, mSparseIntArrayType)) {
            return new SerializationMethods(
                "writeSparseIntArray", false, true, "createSparseIntArray", null, true);
          } else if (mTypes.isAssignable(type, mSparseLongArrayType)) {
            return new SerializationMethods(
                "writeSparseLongArray", false, true, "createSparseLongArray", null, true);
          } else if (mTypes.isAssignable(mTypes.erasure(type), mTypes.erasure(mSparseArrayType))) {
            List<? extends TypeMirror> typeArgs = dt.getTypeArguments();
            if (typeArgs.isEmpty() || mTypes.isSameType(mSparseArrayType, type)) {
              ...
              if (mTypes.isAssignable(typeArg, mBooleanType)) {
                ...
                return new SerializationMethods(
                    "writeFloatSparseArray", false, true, "createFloatSparseArray", null, true);
              } else if (mTypes.isAssignable(typeArg, mDoubleType)) {
                return new SerializationMethods(
                    "writeDoubleSparseArray", false, true, "createDoubleSparseArray", null, true);
              } else if (mTypes.isAssignable(typeArg, mStringType)) {
                return new SerializationMethods(
                    "writeStringSparseArray", false, true, "createStringSparseArray", null, true);
              } else if (mTypes.isSameType(typeArg, mParcelType)) {
                return new SerializationMethods(
                    "writeParcelSparseArray", false, true, "createParcelSparseArray", null, true);
              } else if (mTypes.isAssignable(typeArg, mIBinderType)) {
                return new SerializationMethods(
                    "writeIBinderSparseArray", false, true, "createIBinderSparseArray", null, true);
              } else if (mTypes.isAssignable(typeArg, mParcelableType)) {
                ...
                final TypeMirror internalComponentType = ((ArrayType) typeArg).getComponentType();
      ...
      case ARRAY:
        {
          ...
          switch (componentType.getKind()) {
            case BOOLEAN:
              return new SerializationMethods(
                  "writeBooleanArray", false, true, "createBooleanArray", null, true);
            case BYTE:
              return new SerializationMethods(
            ...
            case DOUBLE:
              return new SerializationMethods(
                  "writeDoubleArray", false, true, "createDoubleArray", null, true);
            case DECLARED:
              {
                final DeclaredType dt = (DeclaredType) componentType;
                if (mTypes.isSameType(componentType, mStringType)) {
                  return new SerializationMethods(
                      "writeStringArray", false, true, "createStringArray", null, true);
                } else if (mTypes.isSameType(componentType, mParcelType)) {
                  return new SerializationMethods(
                      "writeParcelArray", false, true, "createParcelArray", null, true);
                } else if (mTypes.isSameType(componentType, mBigIntegerType)) {
                  return new SerializationMethods(
                      "writeBigIntegerArray", false, true, "createBigIntegerArray", null, true);
                } else if (mTypes.isSameType(componentType, mBigDecimalType)) {
                  return new SerializationMethods(
                      "writeBigDecimalArray", false, true, "createBigDecimalArray", null, true);
                } else if (mTypes.isAssignable(componentType, mIBinderType)) {
                  return new SerializationMethods(
                      "writeIBinderArray", false, true, "createIBinderArray", null, true);
            ...
            case ARRAY:
              {
                ...
                switch (internalComponentType.getKind()) {
                  case BYTE:
                    return new SerializationMethods(
                        "writeByteArrayArray", false, true, "createByteArrayArray", null, true);
                  default:
                    return null;
                }
              }
  ...
  private boolean hasCreator(TypeElement parcelableClass, String generatedClassName) {
    List<VariableElement> fields = ElementFilter.fieldsIn(parcelableClass.getEnclosedElements());
    for (VariableElement field : fields) {
      if ("CREATOR".equals(field.getSimpleName().toString())) {
        ...
        String detectedCreatorTypeName =
            mElements.getPackageOf(parcelableClass).getQualifiedName() + "." + field.asType();
        ...
        if (generatedClassName.equals(detectedCreatorTypeName)
            || expectedAlternativeCreatorTypeName.equals(detectedAlternativeCreatorTypeName)) {
  ...
  private void generateParser(ParcelableClass cl) {
    ...
    try {
      JavaFileObject file =
          this.processingEnv.getFiler().createSourceFile(cl.generatedClassName, cl.parcelableClass);
      ...
      if (cl.requiresApi != null) {
        annotations.setValue(
            "0",
            String.format(Locale.ROOT, "@androidx.annotation.RequiresApi(%d)", cl.requiresApi));
      }
      ...
      mMessager.printMessage(
          Diagnostic.Kind.ERROR,
          "Error writing class file for " + cl.generatedClassName + ": " + ex.getMessage());
    } finally {
      if (writer != null) {
        writer.close();
      }
